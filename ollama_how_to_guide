...
The integration requires foundational tools (Docker and Python) to be
installed first, followed by running the core LLM stack (Ollama/Open
WebUI) and finally setting up the MCP tool chain (Vale/Vale-MCP/mcpo).

**Note:** I followed this guide to setup a locally running AI with the
Vale MCP server and adapted it for the Vale-MCP use case:
...
> uv example: powershell -ExecutionPolicy ByPass -c \"irm
> <https://astral.sh/uv/install.ps1> \| iex"

### My file structure:

![](media/image2.png){width="7.135471347331584in"
...
Easiest method is using a single container image that bundles both
Open WebUI and Ollama

For CPU Only: *docker run -d -p 8080:8080 -v open-webui:/app/data
\--name open-webui ghcr.io/open-webui/open-webui:ollama*

**Note:** I used 8080 to avoid conflicts with the port assigned to
Docker Desktop (it was using port 3000)

2. **Run the container**

In PowerShell, run -\> *docker compose up --d*

\* uses *docker-compose.yml* file which sets up both **Ollama** and
**Open WebUI** cleanly on Windows 11 and makes both services
persistent.
...
3.  **Access the interface**: Once running, Open WebUI is typically
...
**Note:** Docker-compose.yml and start-ollama-background.ps1 files can
be found here:

*\\Mondi-Tech\\Everyone - BSS\\HPE MPC Common\\AI_tools\\*

## Install the MCP tooling chain (Vale, Vale-MCP, mcpo)

1. **Install Vale**

Install Vale CLI on your machine, typically with a package manger
choco install vale
...
**Note:** I had to modify the package.json file to get it to install
without errors

from *\"build\": \"tsc && chmod 755 build/index.js\",* to *\"build\": \"tsc && node -e*

***\\\"try{require(\'fs\').chmodSync(\'build/index.js\',0o755)}catch(e){}\\\"\",***

3.  **Install the mcpo proxy**: *uv tool install mcpo* (to install it
    globally)
...
In PowerShell, go to directory where Vale-MCP is saved and run the
command (again be careful about which port is used):
...
This command starts a web server on a designated port (for example,
8001) as a subprocess for incoming requests. The output should show
the proxy running on an HTTP address such as
[*http://0.0.0.0:8001*](http://0.0.0.0:8001/)

## Configure the tool server in Open WebUI

The proxy's OpenAPI endpoint must be added to the Open WebUI
interface.

In Open WebUI navigate to Settings -\> External Tools -\> Manage Tools
...
- Enter URL where Vale MCP server is running:
  ...
  (this URL is used since Open WebUI is containerizes while mcpo proxy
  runs on your laptop)
...
In Open WebUI navigate to Settings Admin panel -\> Settings -\>
External Tools -\> Manage Tool Servers -\> Add Connection (+)
...
In the main chat window, choose which model to use from the drop-down
box.

2. **Enable the tool**

![](media/image6.png){width="1.1772353455818023in"
height="0.8855325896762904in"}At the bottom of the message box in the
main chat window click Integrations -\> Tools and toggle on the newly
configured tool (Vale MCP)

![](media/image7.png){width="1.7606594488188976in"
height="0.8021894138232721in"}**Note** that the wrench button at the
bottom of the message box shows available tools and the options for
each tool.

3.  **Trigger a tool call**: Send a query to the LLM that requires the
    tool, such as "Use the tool_vale_status_post to check if Vale is
    installed."

4.  If successful, the LLM will respond with information supplied the
    local MCP server, confirming that the three core services (Open
    WebUI, mcpo, MCP server) are communication correctl.
...
2.  Use tool_check_text_post to lint the following text: "Daboll and his
    first defensive coordinator, Wink Martindale, had a falling out and
    They parted ways after 2 years together."

3.  Use tool_check_file_post to lint the following file:
    d:\\hcltech_git\\cms-techdocs-nwdaf\\user_guide\\decision_tables_c.dita

4.  \[Follow up to prompt to #3 since the model didn't provide the
    correct results for prompt #3\] The file is a Dita file and I would
    like it checked against Vale style rules based on the Vale config
    file using tool_check_file_post similar to how you used
    tool_check_text_post
